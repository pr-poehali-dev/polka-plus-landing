...
export default function CableCalculator(){
  ...
  const addMaterial=()=>{const name=newMat.trim();if(!name)return;if(materials.includes(name)){alert(`Материал "${name}" уже есть в списке`);return;}setMaterials(p=>[...p,name]);const enrich=(arr:typeof my)=>arr.map(c=>({...c,recipe:{...c.recipe,[name]:0}}));setMy(enrich);setForeign(enrich);setNewMat("")};
